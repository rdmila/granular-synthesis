...
int main(int, char **) {
  ...
  glfwInitHint(GLFW_PLATFORM, GLFW_PLATFORM_WAYLAND); // TODO: if
  ...
  GLuint spectrogram_texture =
      make_spectrogram_texture(texture_width, texture_height);
...
#endif
  {
    ...
    ImVec2 image_size(texture_width, texture_height);
    ImGui::InvisibleButton("spectrogram_btn", image_size);

    ImVec2 min = ImGui::GetItemRectMin();
    ImVec2 max = ImGui::GetItemRectMax();

    ImDrawList *draw_list = ImGui::GetWindowDrawList();
    draw_list->AddImage((ImTextureID)(intptr_t)spectrogram_texture, min, max);

    if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(0)) {
      ImVec2 mouse_pos = ImGui::GetIO().MousePos;

      ImVec2 local;
      local.x = mouse_pos.x - min.x;
      local.y = mouse_pos.y - min.y;

      points.push_back(local);
    }

    for (size_t i = 0; i < points.size(); i++) {
      ImVec2 p_screen = ImVec2(min.x + points[i].x, min.y + points[i].y);

      draw_list->AddCircleFilled(p_screen, 4.0f, IM_COL32(255, 0, 0, 255));

      if (i > 0) {
        ImVec2 prev = ImVec2(min.x + points[i - 1].x, min.y + points[i - 1].y);

        draw_list->AddLine(prev, p_screen, IM_COL32(0, 255, 0, 255), 2.0f);
      }
    }

    // ImGui::Image((ImTextureID)(intptr_t)spectrogram_texture,
    //              ImVec2(texture_width, texture_height));
    ImGui::End();
